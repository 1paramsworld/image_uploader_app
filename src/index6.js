...
app.get("/user/:email/displayimages", async (req, res) => {
  ...
  try {
    ...
    // Fetch all images for the user's email
    const userImages = await collection.find({ userEmail: userEmail }).toArray();

    // Render the displayimages.hbs template with the images
...
// Add the route to handle Google authentication
